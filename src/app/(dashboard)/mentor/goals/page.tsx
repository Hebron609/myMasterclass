...
// Mock data for mentees' goals
const mockMenteeGoals: Goal[] = [
  {
    id: "1",
    userId: "mentee-1",
...
export default function MentorGoalsPage() {
  ...
  // Calculate stats
  const totalGoals = mockMenteeGoals.length;
  const inProgressCount = mockMenteeGoals.filter(
    (g) => g.status === "IN_PROGRESS",
  ).length;
  const completedCount = mockMenteeGoals.filter(
    (g) => g.status === "COMPLETED",
  ).length;
  const notStartedCount = mockMenteeGoals.filter(
    ...
    <div className="min-h-screen">
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[26px] font-bold text-[#003F3A] mb-2 font-dm-sans">
              Mentee Goals
            </h1>
            <p className="text-[#003F3A] font-rubik text-[16px] font-regular">
              Review and provide feedback on mentee goals
            </p>
          </div>
          <button
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          {/* Goals List */}
          <div className="px-8">
            {mockMenteeGoals.map((goal) => (
              <GoalItem key={goal.id} goal={goal} role="MENTOR" />
            ))}
          </div>
        </div>
